...
# define try \
    ...
    __ctx __ctx_tmp; \
    __err_stk_push(&__ctx_tmp); \
    __err_try = __ctx_save(&__ctx_tmp); \
    if (__err_try == 0)
...
# define throw(x) \
    {while(__now_gen->__err_stk_head == 0 && __now_gen->caller) __now_gen = __now_gen->caller; \
    if(__now_gen->__err_stk_head != 0) __ctx_restore(__err_stk_pop(), x ? x : ERR_THROW0);}
